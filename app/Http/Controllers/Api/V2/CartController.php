...
class CartController extends Controller
{
    ...
    $total_tax = 0.00;
            ...
            foreach ($owner_ids as $owner_id) {
                ...
                if (!empty($shop_items_raw_data)) {
                    foreach ($shop_items_raw_data as $shop_items_raw_data_item) {
						...
						$total_shipping_cost += (float) $shop_items_raw_data_item["shipping_cost"];
                    	$total_tax += $tax;
                        $sub_total += $price + $tax;
        ...
        return response()->json([
			"result" => true,
			"message" => "translate('Cart Details Return Successfully')",
            "grand_total" => single_price($grand_total),
			"total_shipping_cost" => single_price($total_shipping_cost),
     		"total_tax" => single_price($total_tax),
            "data" => $shops
        ]);
    }
